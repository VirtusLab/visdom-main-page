...
Choices worth knowing before changing them:
  ...
  * Equirectangular, because the artwork it replaces reads as equirectangular
    and because it makes the projection two additions (see project() in the
    generated file) rather than a library.
  * Cropped at 84N and 60S, the way every web world map crops Antarctica.
  * Simplified at 0.16 degrees, which is the point where the coastline still
    reads as itself at the width this renders and the file stops growing.
  * Islands under 1.2 square degrees are dropped: they were single grey pixels.
"""
import json
import math
...
def project(point):
    return (point[0] + 180.0, LAT_TOP - point[1])
...
def cut_antimeridian(points):
    ...
    for prev, cur in zip(closed, closed[1:]):
        if abs(cur[0] - prev[0]) > 180:
            y = (prev[1] + cur[1]) / 2
            leaving = 360.0 if prev[0] > cur[0] else 0.0
            current.append((leaving, y))
            pieces.append(current)
            current = [(360.0 - leaving, y), cur]
        else:
            current.append(cur)
    pieces.append(current)
